...
for (let i = 1; i <= letterQuantity.number; i++) {
    ...
    switch (ordinal) {
        case "1" || "21" || "31": ordinal = chalk.red(`${i}st`); break;
        case "2" || "22" || "32": ordinal = chalk.red(`${i}nd`); break;
        case "3" || "23" || "33": ordinal = chalk.red(`${i}rd`); break;
        default: ordinal = chalk.red(`${i}th`); break;
    }
...
const first: string = chalk.yellow.italic.bold("     | A | E | I | M | Q | U | Y |")
const second:string = chalk.yellow.italic.bold("     | B | F | J | N | R | V | Z |")
const third: string = chalk.yellow.italic.bold("     | C | G | K | O | S | W |   |")
const fourth:string = chalk.yellow.italic.bold("     | D | H | L | P | T | X |   |")
firstNumber.forEach((val) => {
    switch (val) {
        ...
        case 4: firstNames.push(fourth); secondNames.push(sample4); break;
        default: return null;
    }
})
...
let Name: string = chalk.red.italic.bold(finalName.join(""))
